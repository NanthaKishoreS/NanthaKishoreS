# NanthaKishores
# 👋 Hi there, I'm Nantha Kishore S!

🎓 I'm a first-year student at **Sri Venkateswara College of Engineering**, passionate about software development and problem-solving.

💻 Currently exploring core **Object-Oriented Programming (OOP)** concepts and diving into **Java**.

🌱 Learning **Java** for the past few weeks and have a foundational understanding of **C/C++**.

🐧 Gaining hands-on experience with **Linux command-line tools** and enjoy working in the **Ubuntu** environment.

📫 You can connect with me on [LinkedIn](https://www.linkedin.com/in/snanthakishore06)

---

## 📈 GitHub Stats

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=NanthaKishoreS&layout=compact&theme=github_dark)
